    ...
    for approach in [baseline] + other:
        with open(os.path.join(output_path, approach, "summary.json")) as f:
            for dataset, result in json.load(f).items():
                ...
                if (
                    "evaluated_rewards" in result
                    and len(result["evaluated_rewards"]) > 0
                ):
                    temp = (
                    ...
                    data[dataset][approach] = 0.0

                if (
                    "graph_generation_time" in result
                    and "space_generation_time" in result
                ):
                    data[dataset][f"argumentation_time_{approach}"] = (
                        result["graph_generation_time"]
                        + result["space_generation_time"]
                    )
                else:
                    data[dataset][f"argumentation_time_{approach}"] = 0

                if "optimization_time" in result and "mining_time" in result:
                    data[dataset][f"automl_time_{approach}"] = (
                        result["optimization_time"] + result["mining_time"]
                    )
                else:
                    data[dataset][f"automl_time_{approach}"] = 0
                ...
                if (
                    ...
                    data[dataset][f"best_time_{approach}"] = result["best_config"][
                        "time"
                    ]
                    ...
                    data[dataset][f"iteration_{approach}"] = 0
                    data[dataset][f"best_time_{approach}"] = 0
                    data[dataset][f"tot_iteration_{approach}"] = 0
                ...
                if approach != baseline:
                    ...
                    if "optimization_time" in result:
                        ...
                        data[dataset][f"time_{approach}"] = 0

    df = pd.DataFrame.from_dict(data, orient="index")

    # mf = pd.read_csv(
    #     os.path.join("/", "home", "automl", "resources", "dataset-meta-features.csv")
    # )
