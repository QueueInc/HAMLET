...
class Buffer:
    ...
    def __new__(cls, metric=None, input_path=None):
        ...
        if metric and input_path:
            cls._instance.loader = Loader(input_path)

            (
                points_to_evaluate,
                evaluated_rewards,
            ) = cls._instance._filter_previous_results(
                cls._instance.loader.get_points_to_evaluate(),
                cls._instance.loader.get_evaluated_rewards(),
                metric,
            )

            cls._instance._num_points_to_consider = len(points_to_evaluate)
            cls._instance._configs = (
                cls._instance.loader.get_instance_constraints() + points_to_evaluate
            )
            ...
            ] * len(cls._instance.loader.get_instance_constraints()) + [
                {
    ...
    def check_template_constraints(self, config):
        for constraint in self.loader.get_template_constraints():
